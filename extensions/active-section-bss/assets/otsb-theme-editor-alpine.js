...
if (!window.otsb.loadedScript.includes('otsb-product-cart.min')) {
  window.otsb.loadedScript.push('otsb-product-cart.min');
    ...
    document.addEventListener('alpine:init', () => {
      Alpine.data('xProductCart', () => ({
        ...
        addToCart(e, buttonElement) {
          ...
          fetch('/cart/add', {
            ...
            if (response.status == '422') {
              ...
              const sectionElement = buttonElement.parentNode.querySelector('.mess_success');
              sectionElement.innerHTML = 'You added ['+ response.product_title + '] to your shopping cart';
              setTimeout(()=>{
        ...
        Updateheader() {
          ...
          var search_url = `${Shopify.routes.root}`;
          ...
          .then((text) => {
            const resultsMarkup = new DOMParser().parseFromString(text, 'text/html').querySelector('.header').innerHTML;
...
if (!window.otsb.loadedScript.includes('otsb-variant-select.min')) {
  window.otsb.loadedScript.push('otsb-variant-select.min');
      ...
      Alpine.data('xVariantSelect', (
        ...
        _updateVariantSelector() {
          ...
          if (this.cachedResults[cacheKey]) {
            ...
            let url = chooseOption?`${productUrl}?variant=${variantId}&otsb-section_id=${this.handleSectionId}&page=${pageParam}`:`${productUrl}?variant=${variantId}&otsb-section_id=${this.handleSectionId}`
            fetch(url)
              .then((response) => response.text())
              .then((responseText) => {
        ...
        _renderBuyButtons(html) {
          const productForms = document.querySelectorAll(`#product-form-${sectionId}, #product-form-installment-${sectionId}, #product-form-sticky-${sectionId}`);
          const atcSource = document.getElementById('x-atc-button-' + sectionId);
          productForms.forEach((productForm) => {
